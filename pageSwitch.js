...
(function ( ROOT, struct, undefined ) {
    ...
    each( "X Y ".split( " " ), function ( name ) {
        ...
        TRANSITION[ 'slice' + name ] = function () {
            var createWrap = function ( node, container ) {
                    ...
                    wrap.style.cssText = 'position:absolute;top:0;left:0;height:100%;width:100%;overflow:hidden;display:block;visibility:visible';
            ...
            var createWrap = function ( node, container, prop, off ) {
                ...
                fixBlock = function ( cpage, tpage, pages, container ) {
                    each( pages, function ( page ) {
                        if ( page.parentNode == container ) return;
                        if ( cpage != page && tpage != page ) {
                            page.parentNode.style.display = page._clone.parentNode.style.visibility = 'hidden';
                            ...
                            page.parentNode.style.display = page._clone.parentNode.style.visibility = 'visible';
                        }
                    } );
                };
    ...
    struct.prototype = {
        ...
        initStyle: function ( elem ) {
            ...
            each( "position:absolute;top:0;left:0;width:100%;height:100%;display:block;visibility:hidden;".split( ";" ), function ( css ) {
                ret = css.split( ":" );
                style[ ret[ 0 ] ] = ret[ 1 ];
            } );
            return elem;
        ...
        remove: function ( index ) {
            ...
            if ( this.pages.length > 1 && index === 0 ) {
                this.next();
                setTimeout( function () {
                    _this.container.removeChild( _this.pages[ index ] );
                    _this.pages.splice( index, 1 );
                    _this.pageData.splice( index, 1 );
                    _this.length = _this.pages.length;
                    if ( index <= _this.current ) {
                        _this.slide( _this.current = Math.max( 0, _this.current - 1 ) );
                    }
                    return _this;
                }, this.duration + 200 );
            } else {
